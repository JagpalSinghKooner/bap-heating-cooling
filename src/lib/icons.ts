 ...
 *
 * Single source of truth for all SVG icon paths.
 * Components should import from here instead of defining inline icons.
 */
...
export const iconPaths = {
  ...
  // === Air Quality ===
  air: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />',
  ...
  // === Seasonal ===
  snowflake:
    '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v18m0-18l-3 3m3-3l3 3m-3 15l-3-3m3 3l3-3M3 12h18M3 12l3-3m-3 3l3 3m15-3l-3-3m3 3l-3 3" />',
...
/**
 * Get icon path by name, with alias support
 * @param name - Icon name or alias
 * @param fallback - Fallback icon name (default: 'check')
 */
export function getIconPath(name: string, fallback: IconName = 'check'): string {
  // Check direct match first
  if (name in iconPaths) {
    return iconPaths[name as IconName];
  }
  // Check aliases
  if (name in iconAliases) {
    return iconPaths[iconAliases[name]];
  }
  // Return fallback
  return iconPaths[fallback];
}
